...
export function CartProvider({ children }) {
  ...
  function getProductQuantity(id) {
   const quantity =  cartProducts.find((product) => product.id === id)?.quantity; //asking for qty of specific id item
  ...
  function addOneToCart(id) {
    ...
    if (quantity === 0) {
      ...
      setCartProducts(
        cartProducts.map(
          (product) =>
            product.id === id // if condition ; take the product we are mapping over and if the product.id is = to id we passing in we going to use ternary operator
    ...
    setCartProducts(
      ...
      (cartProducts) =>
        ...
        cartProducts.filter((currentProduct) => {
          // filter starts with empty [] if an {} meets the condition, add the object to [] 2 != 2 false, if its false its not going to be added to our new array
